    ...
    <div class="container mx-auto p-4">
        ...
        <form action="<?= $route->getLocateAdmin('booking-post-edit', ['id' => $data['booking_id']]) ?>" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="check_in" id="check_in" class="date-picker w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-lg p-3 transition duration-200 ease-in-out hover:border-blue-400" value="<?= date('Y-m-d', strtotime($data['check_in'])) ?>" required>
            </div>
            <div class="mb-4">
                <label for="check_out" class="block text-lg font-medium text-gray-700 mb-2">Ngày Check-out</label>
                <input type="text" name="check_out" id="check_out" class="date-picker w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-lg p-3 transition duration-200 ease-in-out hover:border-blue-400" value="<?= date('Y-m-d', strtotime($data['check_out'])) ?>" required>
            </div>
            <div class="mb-4">
