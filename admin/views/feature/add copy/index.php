...
<div class="max-w-md mx-auto bg-white rounded-lg shadow-md p-6 mt-10">
    <h2 class="text-2xl font-semibold text-gray-700 mb-4">Chỉnh sửa Tiện ích</h2>
    <form action="<?= $route->getLocateAdmin('feature-post-edit', ['id' => $feature['feature_id']]) ?>" method="POST">
        <!-- ID Tiện ích (Ẩn) -->
        <input type="hidden" name="feature_id" value="<?= htmlspecialchars($feature['feature_id']) ?>">
        ...
        <div class="mb-4">
            ...
            <input type="text" 
                   id="feature_name" 
                   name="feature_name" 
                   value="<?= htmlspecialchars($feature['feature_name']) ?>" 
