                ...
                <tr class="hover:bg-gray-100 transition duration-200">
                    ...
                    <td class="py-2 px-4 border-b">
                        ...
                        <div class="flex flex-wrap">
                            ...
                            <?php if ($index < 4): ?>
                            <img src="<?= htmlspecialchars($image) ?>" alt="Hình ảnh phòng"
                                class="w-27 h-24 object-cover rounded m-1">
                            <?php endif; ?>
                            <?php endforeach; ?>
                        </div>
                        <?php else: ?>
                    ...
                    <td class="py-3 px-4 text-sm flex gap-4  items-start">
                        <a href="<?= $route->getLocateAdmin('room-edit', ['id' => $room['room_id'] ]) ?>?>"
                            class="text-blue-500 hover:text-blue-700 hover:underline">Chỉnh sửa</a>
                        <form action="<?= $route->getLocateAdmin('room-delete', ['id' => $room['room_id'] ]) ?>"
                            method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa đánh giá này?');">
                            <button type="submit"
                                class="ml-2 text-red-500 hover:text-red-700 hover:underline">Xóa</button>
                        </form>
                    </td>
                </tr>
                <?php endforeach; ?>
