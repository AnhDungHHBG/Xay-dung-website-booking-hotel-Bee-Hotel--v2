    ...
    <div class="grid grid-cols-3 px-10">
        ...
        <div class="max-w-lg mx-auto bg-white p-5 rounded-lg shadow-lg mb-5">
            <div class="flex items-center">
                ...
                <div class="ml-5">
                    <h2 class="text-xl font-bold">Room code:
                        <?php echo $room['room_id']; ?>-<?php echo $room['room_type']; ?></h2>
            ...
            <form action="<?= $route->getLocateClient('post-reviews', ['room_id' => $room['room_id']]) ?>"
                id="form<?= $room['room_id'] ?>" method="POST" class="mt-4">
                <label class="block text-gray-700">Your rating:</label>
                <div class="rating-container" data-room-id="<?php echo $room['room_id']; ?>">
                    <?php for ($i = 1; $i <= 5; $i++) : ?>
                    <input type="radio" id="star<?php echo $i . '_' . $room['room_id']; ?>" name="rating"
                        value="<?php echo $i; ?>" class="star-rating hidden" />
                ...
                <textarea id="comment" name="comment" rows="4" class="w-full p-2 border border-gray-300 rounded-md"
                    placeholder="Chia sẻ cảm nhận của bạn về phòng"></textarea>
                <button type="submit"
                    class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Review</button>
            </form>
        </div>
        <?php endforeach; ?>
    </div>
    ...
    stars.forEach(star => {
        star.addEventListener('change', () => {
            ...
            labels.forEach(label => {
                const starValue = label.getAttribute('for').split('_')[0].replace(
                    'star', '');
