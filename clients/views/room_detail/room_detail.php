...
<div class="bg-gray-100">
    ...
    <main class="container mx-auto px-6 py-8">
        ...
        <div class="grid grid-cols-3 gap-6">
          ...
          ?>
                <!-- Room Main Image -->
                <div class="h-64 bg-gray-300 rounded-lg shadow-sm"
                    style="background-image: url('<?= htmlspecialchars($images[0]) ?>'); background-size: cover; background-position: center;">
                </div>
            ...
            <!-- "More Photos" Section -->
            <div class="h-64 bg-gray-300 flex justify-center items-center text-gray-500 rounded-lg shadow-md"
                style="background-image: url('<?= htmlspecialchars($images[count($images) - 1]) ?>')">
                <?php
          if (count($images) > 4) {
              echo '+' . (count($images) - 4);
          }
        ...
        <section class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="md:col-span-2">
                ...
                <h1 class="text-3xl font-semibold text-[#133E87]"> Room code:
                    <?= htmlspecialchars($room['room_id']) ?>-<?= htmlspecialchars($room['room_type']) ?></h1>
                ...
                <div class="flex items-center space-x-6 mt-6">
                    <div class="text-gray-600 flex items-center space-x-2">
                        <h2 class="text-xl font-bold text-[#133E87]">Capacity:
                            <?= htmlspecialchars($room['capacity']) ?> <i class="fa-solid fa-person"></i> </h2>
                    </div>
                </div>
            ...
            <div>
                <div class="p-6 bg-white rounded-lg shadow-xl">
                    <h2 class="text-3xl text-center  font-bold text-[#133E87]">$<?= number_format($room['price'], 2) ?>
                        per night</h2>
                    ...
                    <a href="<?= $route->getLocateClient('room-reserve', ['room_id' => $room['room_id']]) ?>">
                        <button
                            class="bg-[#608BC1] text-white w-full py-3 text-lg font-bold mt-6 rounded-md hover:bg-[#395b89]">Reserve
                            Now</button>
                    </a>
        ...
        <!-- Reviews Section -->
        <section class="mt-12">
            <h2 class="text-2xl font-semibold text-[#133E87] mb-6">Reviews <span class="text-yellow-500">★
